...
class Version1EntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Version1(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Version1(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set UUID_GENERATOR_TEST_VERSION_1_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    version_1_ref01_list_result = version_1_ref01_ent.list(version_1_ref01_match, nil)
    assert version_1_ref01_list_result.is_a?(Array)

    # LOAD
    version_1_ref01_match_dt0 = {}
    version_1_ref01_data_dt0_loaded = version_1_ref01_ent.load(version_1_ref01_match_dt0, nil)
...
def version_1_basic_setup(extra)
  ...
  entid_env_raw = ENV["UUID_GENERATOR_TEST_VERSION_1_ENTID"]
