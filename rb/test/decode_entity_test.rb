...
class DecodeEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set UUIDGENERATOR_TEST_DECODE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    decode_ref01_data_dt0_loaded, err = decode_ref01_ent.load(decode_ref01_match_dt0, nil)
    assert_nil err
...
def decode_basic_setup(extra)
  ...
  entid_env_raw = ENV["UUIDGENERATOR_TEST_DECODE_ENTID"]
