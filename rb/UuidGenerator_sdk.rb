...
class UuidGeneratorSDK
  ...
  def initialize(options = {})
    ...
    utility = UuidGeneratorUtility.new
    @_utility = utility
    ...
    feature_opts = UuidGeneratorHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = UuidGeneratorHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, UuidGeneratorFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    UuidGeneratorUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = UuidGeneratorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = UuidGeneratorHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = UuidGeneratorHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = UuidGeneratorError.new(
        "graphql_error", "UuidGeneratorSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = UuidGeneratorSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
