...
module UuidGeneratorUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = UuidGeneratorResponse.new({})
    ...
    if fetch_err
      ...
      response = UuidGeneratorResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = UuidGeneratorResponse.new(fetched)
    else
