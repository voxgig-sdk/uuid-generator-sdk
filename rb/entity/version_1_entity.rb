# UuidGenerator SDK Version1 entity

require_relative '../utility/struct/voxgig_struct'
require_relative '../core/helpers'

class Version1Entity
  ...
  def make
    opts = @_entopts.dup
    Version1Entity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = UuidGeneratorHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Version1, Hash] the current Version1 data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = UuidGeneratorHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Version1 fields)
  def match_get
  ...
  # Load a single Version1.
  #
  # @param reqmatch [Version1LoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Version1.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Version1, Hash] the loaded Version1; raises UuidGeneratorError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = UuidGeneratorHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # List Version1 items matching the given filter.
  #
  # @param reqmatch [Version1ListMatch, Hash, nil] match filter (any subset of
  #   Version1 fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Version1>, Array] the matching Version1 items; raises UuidGeneratorError on failure
