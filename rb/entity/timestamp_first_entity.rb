...
class TimestampFirstEntity
  ...
  def make
    opts = @_entopts.dup
    TimestampFirstEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.TimestampFirst.load works with no args).
  ...
  #
  # @param reqmatch [TimestampFirstListMatch, Hash, nil] match filter (any subset of
  #   TimestampFirst fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
