...
import (
	...
	omni "github.com/voxgig-sdk/uuid-generator-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.UuidGeneratorError); is {
				out := map[string]any{
					"name":    "UuidGeneratorError",
...
func omniCtx(arg any, client *sdk.UuidGeneratorSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.UuidGeneratorSDK); is {
			client = live
