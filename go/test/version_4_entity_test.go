...
func TestVersion4Entity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Version4(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Version4(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		version4Ref01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.version_4")))
...
func version_4BasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("UUID_GENERATOR_TEST_VERSION_4_ENTID")
	...
	idmapResolved := core.ToMapAny(env["UUID_GENERATOR_TEST_VERSION_4_ENTID"])
