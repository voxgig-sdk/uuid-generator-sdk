...
func TestDecodeEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		decodeRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.decode")))
...
func decodeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("UUID_GENERATOR_TEST_DECODE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["UUID_GENERATOR_TEST_DECODE_ENTID"])
