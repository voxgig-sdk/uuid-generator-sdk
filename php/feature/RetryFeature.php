...
// UuidGenerator SDK retry feature
...
class UuidGeneratorRetryFeature extends UuidGeneratorBaseFeature
{
    ...
    public function init(UuidGeneratorContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (UuidGeneratorContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(UuidGeneratorContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(UuidGeneratorContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
