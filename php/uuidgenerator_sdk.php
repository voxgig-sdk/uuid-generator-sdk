...
class UuidGeneratorSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $utility = new UuidGeneratorUtility();
        $this->_utility = $utility;

        $config = UuidGeneratorConfig::shared_config();
        ...
        $feature_opts = UuidGeneratorHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = UuidGeneratorHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!UuidGeneratorFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, UuidGeneratorFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return UuidGeneratorUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = UuidGeneratorHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = UuidGeneratorHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = UuidGeneratorHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
        ...
        $ctx->spec = new UuidGeneratorSpec([
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new UuidGeneratorError($op . "_allow",
                "UuidGeneratorSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = UuidGeneratorHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new UuidGeneratorError("graphql_error",
                "UuidGeneratorSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    // Canonical facade: $client->Decode()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->decode()
    // resolves here too.
    ...
    // Canonical facade: $client->TimestampFirst()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->timestamp_first()
    // resolves here too.
    ...
    // Canonical facade: $client->Version1()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->version_1()
    // resolves here too.
    ...
    // Canonical facade: $client->Version3()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->version_3()
    // resolves here too.
    ...
    // Canonical facade: $client->Version4()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->version_4()
    // resolves here too.
    ...
    // Canonical facade: $client->Version5()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->version_5()
    // resolves here too.
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new UuidGeneratorSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
