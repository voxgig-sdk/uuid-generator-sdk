...
// UuidGenerator SDK feature test
//
...
class FtCtrl extends UuidGeneratorControl
{
...
class FtClient
{
    ...
    private UuidGeneratorUtility $utility;

    public function __construct(UuidGeneratorUtility $utility, array $options)
    {
    ...
    public function get_utility(): UuidGeneratorUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (UuidGeneratorContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public UuidGeneratorContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (UuidGeneratorContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = UuidGeneratorConfig::make_feature($name);
    ...
    private static function buildUrl(UuidGeneratorSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof UuidGeneratorError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new UuidGeneratorResponse($fetched);
        }
    ...
    private function fail(UuidGeneratorContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (UuidGeneratorContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
