...
class Version1EntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = UuidGeneratorSDK::test(null, null);
        $ent = $testsdk->Version1(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Version1(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Version1(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = version_1_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "version_1." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set UUID_GENERATOR_TEST_VERSION_1_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $version_1_ref01_data = null;
        if (count($version_1_ref01_data_raw) > 0) {
...
function version_1_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/version_1/Version1TestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["version_101", "version_102", "version_103", "count01", "count02", "count03"] as $k) {
    ...
    $entid_env_raw = getenv("UUID_GENERATOR_TEST_VERSION_1_ENTID");
