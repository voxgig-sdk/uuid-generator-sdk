...
class DecodeEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = UuidGeneratorSDK::test(null, null);
        $ent = $testsdk->Decode(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = decode_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "decode." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set UUID_GENERATOR_TEST_DECODE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $decode_ref01_data = null;
        if (count($decode_ref01_data_raw) > 0) {
...
function decode_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/decode/DecodeTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["decode01", "decode02", "decode03"] as $k) {
    ...
    $entid_env_raw = getenv("UUID_GENERATOR_TEST_DECODE_ENTID");
