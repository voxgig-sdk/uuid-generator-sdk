...
describe("TimestampFirstDirect", function()
  it("should direct-list-timestamp_first", function()
    local setup = timestamp_first_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-timestamp_first", setup.live and "live" or "unit")
  ...
  it("should direct-load-timestamp_first", function()
    local setup = timestamp_first_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-timestamp_first", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function timestamp_first_direct_setup(mockres)
