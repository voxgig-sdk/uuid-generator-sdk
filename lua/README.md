...
The Lua SDK for the UuidGenerator API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Decode(nil):load({ id = "example_id" }, nil)
...
local result, err = client:UuidGenerator(nil):load(
  { id = "test01" }, nil
)
