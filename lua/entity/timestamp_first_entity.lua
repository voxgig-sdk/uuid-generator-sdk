...
local TimestampFirstEntity = {}
TimestampFirstEntity.__index = TimestampFirstEntity


function TimestampFirstEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TimestampFirstEntity)
  self._name = "timestamp_first"
  self._client = client
...
function TimestampFirstEntity:get_name()
  return self._name
end


function TimestampFirstEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TimestampFirstEntity.new(self._client, opts)
end
...
function TimestampFirstEntity:mark_deleted()
  self._deleted = true
end


function TimestampFirstEntity:deleted()
  return true == self._deleted
end


function TimestampFirstEntity:data_set(args)
  if args ~= nil then
...
function TimestampFirstEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TimestampFirstEntity:match_set(args)
  if args ~= nil then
...
function TimestampFirstEntity:match_get()
...
function TimestampFirstEntity:stream(action, args, callopts)
...
function TimestampFirstEntity:_run_op(ctx, post_done)
  local utility = self._utility
