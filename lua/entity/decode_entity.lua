-- UuidGenerator SDK Decode entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local DecodeEntity = {}
DecodeEntity.__index = DecodeEntity


function DecodeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DecodeEntity)
  self._name = "decode"
  self._client = client
...
function DecodeEntity:get_name()
  return self._name
end


function DecodeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DecodeEntity.new(self._client, opts)
end
...
function DecodeEntity:mark_deleted()
  self._deleted = true
end


function DecodeEntity:deleted()
  return true == self._deleted
end


function DecodeEntity:data_set(args)
  if args ~= nil then
...
function DecodeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DecodeEntity:match_set(args)
  if args ~= nil then
...
function DecodeEntity:match_get()
...
function DecodeEntity:stream(action, args, callopts)
...
---@param reqmatch DecodeLoadMatch
---@param ctrl? table
---@return Decode
---@return string? err
function DecodeEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DecodeEntity:_run_op(ctx, post_done)
  local utility = self._utility
