...
local Version1Entity = {}
Version1Entity.__index = Version1Entity


function Version1Entity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, Version1Entity)
  self._name = "version_1"
  self._client = client
...
function Version1Entity:get_name()
  return self._name
end


function Version1Entity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return Version1Entity.new(self._client, opts)
end
...
function Version1Entity:mark_deleted()
  self._deleted = true
end


function Version1Entity:deleted()
  return true == self._deleted
end


function Version1Entity:data_set(args)
  if args ~= nil then
...
function Version1Entity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function Version1Entity:match_set(args)
  if args ~= nil then
...
function Version1Entity:match_get()
...
function Version1Entity:stream(action, args, callopts)
...
---@param reqmatch Version1LoadMatch
---@param ctrl? table
---@return Version1
---@return string? err
function Version1Entity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch Version1ListMatch
---@param ctrl? table
---@return Version1[]
---@return string? err
function Version1Entity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function Version1Entity:_run_op(ctx, post_done)
  local utility = self._utility
