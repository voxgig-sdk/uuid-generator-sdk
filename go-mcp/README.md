...
MCP server exposing the UuidGenerator SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `uuid-generator_list` | `entity`, optional `query` map | First page of records as JSON |
| `uuid-generator_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `uuid-generator_list` and
`uuid-generator_load` tools become available in new sessions.
