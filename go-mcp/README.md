...
[MCP](https://modelcontextprotocol.io) server exposing the UuidGenerator SDK as
two agent tools — `uuid-generator_list` and `uuid-generator_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/uuid-generator-mcp)
make build
...
claude mcp add --scope user uuid-generator \
  -- /absolute/path/to/uuid-generator-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./uuid-generator-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// uuid-generator_list: first page of records
{ "entity": "timestamp_first" }
{ "entity": "timestamp_first", "query": { } }

// uuid-generator_load: one record by id
{ "entity": "decode", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/uuid-generator-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user uuid-generator \
     -- "$PWD"/dist/*/uuid-generator-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `uuid-generator_list` and `uuid-generator_load` tools now appear
   in new sessions. Ask the agent to *"list timestamp_first using uuid-generator"*
   and it calls `uuid-generator_list` with `{"entity":"timestamp_first"}`.
...
### Call the `uuid-generator_list` tool
...
### Call the `uuid-generator_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `uuid-generator_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `uuid-generator_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./uuid-generator-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"uuid-generator_load","arguments":{"entity":"decode","query":{"id":1}}}}'
```
