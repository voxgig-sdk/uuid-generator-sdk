...
//     (\`new UuidGeneratorSDK(...)\` / \`UuidGeneratorSDK.test(...)\` become
//     \`UuidGeneratorSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'UuidGeneratorSDK'
...
const TEST_SEED = {"entity":{"decode":{"test01":{"id":"test01"}},"timestamp_first":{"test01":{"id":"test01"}},"version_1":{"test01":{"id":"test01"}},"version_3":{"test01":{"id":"test01"}},"version_4":{"test01":{"id":"test01"}},"version_5":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new UuidGeneratorSDK(...)\` and any
// \`UuidGeneratorSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+UuidGeneratorSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/UuidGeneratorSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
