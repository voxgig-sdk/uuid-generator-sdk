...
from uuidgenerator_sdk.uuidgenerator_types import (
    TimestampFirst,
...
class TimestampFirstEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TimestampFirstEntity(self._client, opts)
    ...
    def data_get(self) -> TimestampFirst:
    ...
    def match_get(self) -> TimestampFirst:
    ...
    def load(self, reqmatch=None, ctrl=None) -> TimestampFirst:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.TimestampFirst().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[TimestampFirst]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.TimestampFirst().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
