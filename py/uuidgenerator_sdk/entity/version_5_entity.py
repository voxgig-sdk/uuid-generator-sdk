...
class Version5Entity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return Version5Entity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> Version5:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Version5().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
