...
class UuidGeneratorTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            UuidGeneratorTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = UuidGeneratorTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = UuidGeneratorTestRunner.getenv("UUID_GENERATOR_TEST_LIVE")
        override = UuidGeneratorTestRunner.getenv("UUID_GENERATOR_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = UuidGeneratorTestRunner.getenv(key)
        ...
        explain = UuidGeneratorTestRunner.getenv("UUID_GENERATOR_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if UuidGeneratorTestRunner._test_control is not None:
            return UuidGeneratorTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                UuidGeneratorTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            UuidGeneratorTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = UuidGeneratorTestRunner.load_test_control()
...
def load_env_local():
    UuidGeneratorTestRunner.load_env_local()


def env_override(m):
    return UuidGeneratorTestRunner.env_override(m)


def entity_data(v):
    return UuidGeneratorTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return UuidGeneratorTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return UuidGeneratorTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return UuidGeneratorTestRunner.load_test_control()


def live_client_options():
    return UuidGeneratorTestRunner.live_client_options()


def live_delay_ms():
    return UuidGeneratorTestRunner.live_delay_ms()
