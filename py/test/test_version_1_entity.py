...
class TestVersion1Entity:

    def test_should_create_instance(self):
        testsdk = UuidGeneratorSDK.test(None, None)
        ent = testsdk.Version1(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Version1(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Version1(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _version_1_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "version_1." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set UUID_GENERATOR_TEST_VERSION_1_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        version_1_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.version_1")))
        version_1_ref01_data = None
        if len(version_1_ref01_data_raw) > 0:
...
def _version_1_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/version_1/Version1TestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("UUID_GENERATOR_TEST_VERSION_1_ENTID"))
